...
class INotionSchemaManager(ABC):
    ...
    async def ensure_select_options(
        self, database_id: str, property_name: str, options: List[str]
    ) -> List[str]:
...
class ICacheManager(ABC):
    ...
    async def set_cached_data(
        self, key: str, data: Any, ttl_seconds: int = 600
    ) -> bool:
...
class ITaskWorkflowManager(ABC):
    ...
    async def execute_task_creation_workflow(
        self, assignee: str, task_name: str, requester_id: int
    ) -> DiscordMessageResponseDTO:
...
class IMeetingWorkflowManager(ABC):
    ...
    async def execute_meeting_creation_workflow(
        self, title: str, requester_id: int
    ) -> DiscordMessageResponseDTO:
