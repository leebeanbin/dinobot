...
from src.dto.notion.notion_dtos import (
...
from src.dto.webhook.webhook_dtos import (
    NotionWebhookRequestDTO,
    WebhookProcessResultDTO,
)
from src.dto.common.metrics_dtos import (
    CommandExecutionMetricDTO,
    APICallMetricDTO,
    CachePerformanceMetricDTO,
)
from src.dto.common.system_dtos import SystemStatusDTO
from src.dto.notion.request_dtos import (
...
class INotionService(ABC):
    ...
    async def ensure_select_options(
        self, database_id: str, property_name: str, required_options: List[str]
    ) -> List[str]:
        """
    ...
    async def create_page(
        self, request: NotionPageCreateRequestDTO
    ) -> NotionPageResponseDTO:
        """
    ...
    async def create_meeting_minutes(
        self, request: MeetingCreateRequestDTO
    ) -> NotionPageResponseDTO:
        """
...
class IDiscordService(ABC):
    ...
    async def get_or_create_daily_thread(
        self, channel_id: int, date_str: str
    ) -> ThreadInfoDTO:
        """
...
class IWebhookService(ABC):
    ...
    async def process_notion_webhook(
        self, request: NotionWebhookRequestDTO
    ) -> WebhookProcessResultDTO:
        """
    ...
    async def verify_webhook_security(
        self, secret: str, request_data: Dict[str, Any]
    ) -> bool:
        """
...
class IBusinessService(ABC):
    ...
    async def execute_task_creation_workflow(
        self, assignee: str, task_name: str, requester_user_id: int, channel_id: int
    ) -> DiscordMessageResponseDTO:
    ...
    async def execute_meeting_creation_workflow(
        self, title: str, requester_user_id: int, channel_id: int
    ) -> DiscordMessageResponseDTO:
    ...
    async def execute_webhook_summary_workflow(
        self, page_id: str, channel_id: int
    ) -> WebhookProcessResultDTO:
