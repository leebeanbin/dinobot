...
from src.core.database import schema_cache_manager, metrics_collector
from src.core.logger import get_logger, logger_manager
from src.core.exceptions import NotionAPIException, safe_execution
from src.core.decorators import track_notion_api

# Interface will be properly implemented in future refactoring
# from models.interfaces import INotionService
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"⚠️ Notion API 오류 (시도 {attempt + 1}/{max_retries}): {error_str}, {wait_time:.2f}초 후 재시도"
                        )
                        await asyncio.sleep(wait_time)
                    else:
                        logger.error(
                            f"❌ Notion API 재시도 실패 (최대 {max_retries}회): {error_str}"
                        )
...
class NotionService:
    ...
    def __init__(self):
        # Notion API 2025-09-03 버전으로 업그레이드
        self.notion_api_client = NotionClient(
            auth=settings.notion_token, notion_version="2025-09-03"
        )
        # Notion service manager initialization complete (로그 제거)
    ...
    async def get_database_schema(self, notion_db_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 노션 API에서 최신 스키마 가져오기 (새로운 API 버전 사용)
            with logger_manager.performance_logger("notion_schema_api_call"):
                # 새로운 API 버전에서는 search를 통해 data_source 검색
                search_response = self.notion_api_client.search(
                    query="", filter={"property": "object", "value": "data_source"}
                )

                # 해당 데이터베이스의 데이터 소스 찾기
                data_sources = []
                for result in search_response.get("results", []):
                    if (
                        result.get("database_parent", {}).get("database_id")
                        == notion_db_id
                    ):
                        data_sources.append(result)

                if not data_sources:
                    # 데이터 소스를 찾을 수 없으면 기존 방법 사용
                    from notion_client import Client as NotionClientLegacy

                    legacy_client = NotionClientLegacy(
                        auth=settings.notion_token, notion_version="2022-06-28"
                    )
                    raw_response = legacy_client.databases.retrieve(
                        database_id=notion_db_id
                    )
                else:
                    # 첫 번째 데이터 소스 사용
                    raw_response = data_sources[0]
    ...
    @safe_execution("get_database_data_sources")
    async def get_database_data_sources(self, database_id: str) -> List[Dict[str, Any]]:
        """
        데이터베이스의 데이터 소스 목록을 가져옵니다 (2025-09-03 API 버전)
        ...
        """
        try:
            response = self.notion_api_client.databases.retrieve(database_id)
            return response.get("data_sources", [])
        except Exception as e:
            logger.error(f"❌ 데이터 소스 조회 실패: {e}")
            return []

    @safe_execution("get_primary_data_source_id")
    async def get_primary_data_source_id(self, database_id: str) -> Optional[str]:
        """
        데이터베이스의 주요 데이터 소스 ID를 가져옵니다

        Args:
            database_id: 노션 데이터베이스 ID

        Returns:
            주요 데이터 소스 ID (첫 번째 데이터 소스)
        """
        data_sources = await self.get_database_data_sources(database_id)
        if data_sources:
    ...
    async def create_schema_based_properties(
        ...
        try:
            ...
            for input_property_name, input_value in user_input_values.items():
                # 실제 속성명 찾기 (정확한 매칭 우선, 그 다음 대소문자 무시)
                actual_property_name = None

                # 1. 정확한 매칭 시도
                if input_property_name in schema["props"]:
                    actual_property_name = input_property_name
                # 2. 대소문자 무시 매칭 시도
                elif input_property_name.lower() in property_name_mapping:
                    actual_property_name = property_name_mapping[
                        input_property_name.lower()
                    ]

                if not actual_property_name:
                    logger.info(f"🤷 알 수 없는 속성 '{input_property_name}' 건너뜀")
                    logger.debug(f"   사용 가능한 속성: {list(schema['props'].keys())}")
                    continue
    ...
    async def create_factory_task(
        ...
        logger.info(
            f"📋 Factory Tracker DB 스키마: {list(schema.get('props', {}).keys())}"
        )
        ...
        if "Person" in schema.get("props", {}):
            ...
            for prop_name, prop_info in schema.get("props", {}).items():
                if (
                    prop_info.get("type") == "select"
                    or prop_info.get("type") == "multi_select"
                ):
        ...
        if priority and (
            "Priority" in schema.get("props", {})
            or "우선순위" in schema.get("props", {})
        ):
            priority_field = (
                "Priority" if "Priority" in schema.get("props", {}) else "우선순위"
            )
        ...
        if task_type:
            if "Task type" in schema.get("props", {}):
                ...
                for prop_name, prop_info in schema.get("props", {}).items():
                    if (
                        prop_info.get("type") == "select"
                        and prop_name not in user_values
                    ):
        ...
        try:
            # 2025-09-03 API 버전: 데이터 소스 ID 사용
            data_source_id = await self.get_primary_data_source_id(
                settings.factory_tracker_db_id
            )
            if not data_source_id:
                raise NotionAPIException("데이터 소스 ID를 찾을 수 없습니다")
    ...
    async def create_meeting_page(
        ...
        # 스키마 확인
        schema = await self.get_database_schema(settings.board_db_id)
        available_props = schema.get("props", {})

        logger.info(f"📋 Board DB 사용 가능한 속성: {list(available_props.keys())}")
        ...
        if "Status" in available_props:
            ...
            logger.info(f"✅ Status 속성 설정: {status_value}")
        else:
            logger.info("⚠️ Status 속성이 없어 건너뜀")

        # Participants 속성이 있으면 설정
        if participants and "Participants" in available_props:
            user_values["Participants"] = participants
            logger.info(f"✅ Participants 속성 설정: {participants}")
        elif participants:
            logger.info("⚠️ Participants 속성이 없어 건너뜀")
        ...
        try:
            # 2025-09-03 API 버전: 데이터 소스 ID 사용
            data_source_id = await self.get_primary_data_source_id(settings.board_db_id)
            if not data_source_id:
                raise NotionAPIException("데이터 소스 ID를 찾을 수 없습니다")
    ...
    async def check_page_exists(self, page_id: str) -> bool:
        ...
        try:
            ...
            if (hasattr(e, "status") and e.status == 404) or any(
                code in error_str
                for code in ["404", "not found", "unauthorized", "forbidden"]
            ):
                logger.debug(
                    f"🔍 페이지 존재하지 않음 또는 접근 불가: {page_id} - {error_str}"
                )
                return False
            else:
                # 다른 API 오류는 일시적인 것으로 간주하고 존재하는 것으로 처리
                logger.warning(
                    f"⚠️ 페이지 존재 확인 중 API 오류: {page_id} - {error_str}"
                )
    ...
    async def extract_page_text(self, page_id: str, use_cache: bool = True) -> str:
        ...
        if use_cache:
            try:
                from src.core.database import get_meetup_collection
        ...
        try:
            ...
            if use_cache and total_blocks > 0:
                try:
                    ...
                    logger.debug(
                        f"💾 페이지 내용 캐시 저장: {page_id} ({total_blocks}블록, {len(full_text)}자)"
                    )
                except Exception as cache_save_error:
                    logger.warning(f"⚠️ 페이지 내용 캐시 저장 실패: {cache_save_error}")

            logger.debug(
                f"📄 페이지 텍스트 추출 완료: {total_blocks}블록, {len(full_text)}자"
            )
            return full_text
