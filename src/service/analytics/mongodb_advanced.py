...
from src.core.exceptions import DatabaseOperationException, safe_execution
from src.core.decorators import track_mongodb_query
...
class MongoDBRealTimeStreaming:
    ...
    async def start_realtime_metrics_stream(self, event_handler: callable):
        ...
        try:
            ...
            try:
                ...
                # 메트릭 실시간 스트리밍 시작 (Change Stream) (로그 제거)
                ...
                if "40573" in error_msg or "replica sets" in error_msg.lower():
                    logger.info(
                        "📡 단일 MongoDB 인스턴스 감지 - 폴링 방식으로 모니터링 대체"
                    )
                else:
    ...
    async def _process_change_events_loop(self, stream_name: str):
        ...
        try:
            ...
            if "40573" in error_msg or "replica sets" in error_msg.lower():
                logger.debug(
                    f"📡 Change Stream 기능 비활성화됨 (단일 MongoDB): {error_msg}"
                )
            else:
