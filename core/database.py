...
class MongoDBConnectionManager:
    ...
    async def _create_required_indexes(self):
        ...
        try:
            ...
            await page_cache_collection.create_index("page_id", unique=True)  # 페이지 ID 고유키
            ...
            logger.info("📈 MongoDB 인덱스 생성 완료 (캐시 및 성능 최적화 포함)")
...
async def initialize_meetup_loader_collections():
    """MeetupLoader 서비스에 필요한 모든 컬렉션을 초기화"""
    logger.info("🔧 MeetupLoader 컬렉션 초기화 시작...")
        ...
        for collection_name, config in MEETUP_LOADER_COLLECTIONS.items():
            try:
                if collection_name in existing_collection_names:
                    logger.info(f"📂 기존 컬렉션 확인: {collection_name}")
                    ...
                    logger.info(
                        f"✅ 새 컬렉션 생성: {collection_name} - {config['description']}"
                    )
...
# ===== MeetupLoader 서비스 전용 데이터 저장 함수들 =====
